...
async function main() {
    console.log("Starting bulk insertion of 200 MORE hymns per category...");
    
    const lastHymn = await prisma.hymn.findFirst({
        orderBy: { number: 'desc' }
    });
    
    let currentNumber = lastHymn ? lastHymn.number + 1 : 1000;
    const hymnsToInsert = [];
    
    for (const [tag, bHymns] of Object.entries(baseHymns)) {
        for (let i = 0; i < 200; i++) {
            const base = bHymns[i % bHymns.length];
            const arrNum = Math.floor(i / bHymns.length) + 11; // start from 11 so it differs from the previous batch
            
            const suffixes = [
               ` (Congregation Arr. ${arrNum})`,
               ` (Acoustic Arr. ${arrNum})`,
               ` (Organ Arr. ${arrNum})`,
               ` (Youth Ver. ${arrNum})`,
               ` (Contemporary ${arrNum})`
            ...
            const lyricsMod = `[Arrangement Note: Variation ${arrNum} - Expanded verses]\n\n` + base.lyrics;
    ...
    console.log("✅ Appended 1000 hymns successfully. Total should now be 250+ per category.");
}
